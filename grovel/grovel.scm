...
(define posix (with-input-from-file "errno-posix.text" read-all))
...
(define (write-c-program)
  ...
  (for-each (lambda (symbol)
              (let ((non-posix? (not (member symbol posix))))
                (when non-posix? (disp "#ifdef " symbol))
                (disp tab "PR(" symbol ");")
                (when non-posix? (disp "#endif"))))
            all-known)
  (disp "}"))
